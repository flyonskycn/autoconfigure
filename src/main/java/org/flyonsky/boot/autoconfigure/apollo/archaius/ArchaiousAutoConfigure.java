...
@AutoConfigureBefore({ArchaiusAutoConfiguration.class})
...
public class ArchaiousAutoConfigure {
	...
	@Bean
	public ArchaiousConfig archaiousConfig(ApolloIdProperties apolloId, ApolloProperties apollo) {
		ArchaiousConfig config = new ArchaiousConfig();
		config.init(apolloId, apollo);
		return config;
	}
	
	public static class ArchaiousConfig {
		public void init(ApolloIdProperties apolloId, ApolloProperties apollo) {
...
//			try {
//				config.addConfiguration(urlConfig, ConfigurationManager.URL_CONFIG_NAME);
//			}catch (Throwable ex)
//			{
//				logger.error("Cannot create config from " + urlConfig, ex);
//	        }
